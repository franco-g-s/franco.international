...
| Site                          | Repo                          | Purpose                                     |
| ----------------------------- | ----------------------------- | ------------------------------------------- |
| `franco.international`        | `franco.international`        | Personal hub — landing page + section links |
| `garden.franco.international` | `garden.franco.international` | Digital garden (Quartz)                     |
| `cv.franco.international`     | `cv.franco.international`     | Standalone CV                               |

**Tech stack:**

- `franco.international` → **Astro** (static, GitHub Pages)
- `cv.franco.international` → **Plain HTML/CSS** (single page, printable)
- `garden.franco.international` → **Quartz v5** (community plugins, YAML config)
...
- [x] Configure DNS: `cv.franco.international` → GitHub Pages
...
- [x] Add JSON feed emitter to Quartz (`plugins/json-feed/`) — outputs `/notes.json` with title, slug, date, description sorted by date
...
- [x] Set up temporary redirect: `franco.international` → `garden.franco.international`
...
- 3 most recent notes pulled from `garden.franco.international/notes.json` at Astro build time
...
## Phase 3.5: Quartz v4 → v5 Migration ✅ **COMPLETE**

### Goal

Upgrade the garden from Quartz v4 to v5, adopting the new YAML config model and community plugin system.

### Key Changes

- `quartz.config.ts` → `quartz.config.yaml` (full site config in YAML)
- `quartz.ts` handles TypeScript-level overrides only (filterFn, custom conditions)
- Community plugins installed via `npx quartz plugin install`, tracked in `quartz.lock.json`
- Custom plugins (JSON feed, frontmatter properties) moved to `plugins/` for future revamp
- Quartz Bases built into v5 (bases-page feature enabled)

---

## Phase 4: Hub Mobile Optimization _(future)_
...
## Phase 5: Digital Garden Revamp _(future)_
...
- Revisit component set using Quartz v5 plugin model
...
Once Phase 5 defines the garden's structure, update the Digital Garden section of `franco.international`:

- **Replace** the "5 most recent notes" feed with the garden's top-level areas / folders / sections
...
- The `notes.json` feed emitter may need to be replaced or supplemented with an `areas.json`
...
| 2026-05-26 | Quartz v4 → v5 upgrade          | v5 community plugin ecosystem; Bases built-in; YAML config   |
